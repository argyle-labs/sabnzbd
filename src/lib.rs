...
/// sabnzbd backend. Holds only the provider name; per-instance endpoint/creds
/// come from the `Endpoint` the generic `service.*` tools hand each op.
#[derive(Debug, Clone)]
pub struct SabnzbdBackend {
    provider: &'static str,
}
...
impl ServiceBackend for SabnzbdBackend {
    fn provider(&self) -> &str { self.provider }

    /// Runtimes sabnzbd can be placed on. `service.deploy` hands the
    ...
    {
        // TODO: describe the sabnzbd workload (image/template, ports, mounts,
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("sabnzbd.workload_spec")) })
    }
        ...
        -> BoxFuture<'a, Result<BackupArtifact, ServiceError>>
    {
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("sabnzbd.backup")) })
    }

    fn restore<'a>(&'a self, _ep: &'a Endpoint, _from: &'a BackupArtifact)
        -> BoxFuture<'a, Result<(), ServiceError>>
    {
        Box::pin(async move { Err(ServiceError::unimplemented("sabnzbd.restore")) })
    }

    fn configure<'a>(&'a self, _ep: &'a Endpoint, _config: &'a str)
        -> BoxFuture<'a, Result<(), ServiceError>>
    {
        // TODO: apply sabnzbd-specific config idempotently.
        Box::pin(async move { Err(ServiceError::unimplemented("sabnzbd.configure")) })
    }
        ...
        -> BoxFuture<'a, Result<ServiceStatus, ServiceError>>
    {
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("sabnzbd.status")) })
    }
}
...
mod tests {
    ...
    fn declares_provider() {
        let b = SabnzbdBackend::new("sabnzbd");
        assert_eq!(b.provider(), "sabnzbd");
    }
}
